"""Step 14: Color palettes and additive blending — named palettes with
keyboard cycling for multi-species rendering.

Builds on Step 13 (multi-species). Changes:
  - Species colors are no longer hardcoded in SPECIES_CONFIGS — they come
    from a named PALETTE selected at runtime
  - 8 built-in palettes: warm, cool, neon, fire, ocean, pastel, sunset, forest
  - 3 species (up from 2) to show off richer territory competition
  - Keyboard: P cycles through palettes live
  - Additive blending (from step 13) means overlapping species create new
    colors at boundaries (e.g. red + blue → purple)

    python src/step_14_color_palettes.py [mode] [palette]
    python src/step_14_color_palettes.py random neon
    python src/step_14_color_palettes.py clusters fire
"""
...
# --- Color palettes (NEW in this step) ---
# Each palette has one RGB tuple per species. Overlapping trails blend
# additively — e.g. neon red + green → yellow at boundaries.
...
# --- Simulation functions (unchanged from step 13) ---
...
# --- Rendering with palette colors (NEW in this step) ---


def draw(screen, grids, palette_colors):
    """Render with additive blending using palette colors.

    Each species' normalized+gamma-corrected intensity is multiplied by that
    species' palette color. Results are summed across species and clamped to 255.
    Where species overlap, their colors mix (e.g. red + blue → purple).
    """
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    running = False
                # Cycle palette with P key (NEW in this step)
